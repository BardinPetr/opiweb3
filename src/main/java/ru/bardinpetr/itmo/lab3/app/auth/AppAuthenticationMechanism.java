...
public class AppAuthenticationMechanism implements HttpAuthenticationMechanism {
    ...
    private static final org.slf4j.Logger log = org.slf4j.LoggerFactory.getLogger(AppAuthenticationMechanism.class);
    ...
    public AuthenticationStatus validateRequest(HttpServletRequest req, HttpServletResponse res, HttpMessageContext ctx) {
        ...
        if (upCred.isPresent()) {
            ...
            if (valid.getStatus() != CredentialValidationResult.Status.VALID) return ctx.doNothing();
        ...
        if (jwtCred.isPresent()) {
            ...
            if (valid.getStatus() != CredentialValidationResult.Status.VALID) return redirectToLogin(req, res, ctx);
            return ctx.notifyContainerAboutLogin(valid);
        }
        if (ctx.getCallerPrincipal() != null) return ctx.doNothing();
        if (ctx.isProtected()) return redirectToLogin(req, res, ctx);
    ...
    private void issueToken(HttpServletResponse response, CredentialValidationResult result) {
        if (result.getStatus() != CredentialValidationResult.Status.VALID) return;
        var jwtPrincipal = new JWTCallerPrincipal(result.getCallerPrincipal().getName(), result.getCallerGroups(), JWTType.ACCESS);
    ...
    private Optional<UsernamePasswordCredential> getLoginCredentials(HttpServletRequest req, HttpServletResponse res, HttpMessageContext ctx) {
        ...
        if (auth.isNewAuthentication() && auth.getCredential() instanceof UsernamePasswordCredential cred) return Optional.of(cred);
