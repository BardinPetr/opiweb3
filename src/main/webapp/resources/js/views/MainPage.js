(() => {
  ...
  // js/utils/utils.js
  var range = (start, end, step = 1) => [...Array(Math.ceil((end - start) / step) + 1).keys()].map(
    (i) => start + i * step
  );
  var absMax = (...arr) => Math.max(...arr.map((i) => Math.abs(i)));
  function toFixedString(val, len) {
  ...
  var BaseCanvas = class {
    ...
    constructor(canvas, onClick) {
      ...
      $(window).on("resize", () => {
        if (this.#sizeUpdateHandler)
          clearTimeout(this.#sizeUpdateHandler);
    ...
    drawArrow([x0, y0], [x1, y1], width = 1, size = 10) {
      this._env((c) => {
        ...
        const delta = (isSin, angleDelta) => size * (isSin ? Math.sin : Math.cos)(angle + angleDelta);
        c.beginPath();
        c.lineWidth = width;
        c.moveTo(x0, y0);
        c.lineTo(x1, y1);
  ...
  var BaseCanvas_default = BaseCanvas;

  // js/ui/PlotDisplay.js
  var AxesDisplay = class extends BaseCanvas_default {
    _ratio = 1;
    _ptsWidth = 0;
    _ptsHeight = 0;
    #targetDimension = 0;
  ...
  var PlotDisplay_default = AxesDisplay;

  // js/views/FigureDisplay.js
  var FigureDisplay = class extends PlotDisplay_default {
    #datastore;
    #disabled = false;
    ...
    addPoint({ x, y, r, result }) {
      const color = Math.abs(r - this.#datastore.getR()) > 1e-5 ? "#AAAAAADD" : result ? "#00FF00" : "#FF0000";
  ...
  var FigureDisplay_default = FigureDisplay;

  // js/data/PointResult.js
  var PointResult = class {
  ...
  var PointResult_default = PointResult;

  // js/data/api.js
  var extractReturn = (x) => x.jqXHR.pfArgs;
  var pointResultFromServer = ({ x, y, r, inside }) => new PointResult_default({ x, y, r, result: inside });
  var checkPointRequest = async (x, y, r) => {
  ...
  var getR = async () => extractReturn(await remoteGetR()).r;
  var extractPoints = (response) => (response.points || []).map(pointResultFromServer);
  var lastTime = 0;
  var getNewPoints = async () => {
  ...
  var getPoints = async () => extractPoints(extractReturn(await remoteGetNewPoints()));
  var getConstraints = async () => {
    const { constraints: [r, x, y] } = extractReturn(await remoteGetConstraints());
    return { r, x, y };
  };

  // js/data/PointResultStorage.js
  var EVENT_SOURCES = ["point-check-form:sendBtn", "point-check-form:rInput", "table:clear-form"];
  var PointResultStorage = class {
    #callbacks = {
      clear: [],
      insert: [],
      config: [],
  ...
  var PointResultStorage_default = PointResultStorage;

  // js/validation/Validator.js
  var Validator = class {
  ...
  // js/validation/FloatValidator.js
  var EPS = 1e-6;
  var isFloat = (x) => /^-?\d+(\.\d+)?$/.test(x);
  var FloatValidator = class extends Validator {
    #min;
    #max;
    #minInclusive;
    #maxInclusive;
    ...
    check(value) {
      ...
      const valid = (this.#maxInclusive ? Math.abs(maxDelta) < EPS || maxDelta > EPS : maxDelta > EPS) && (this.#minInclusive ? Math.abs(minDelta) < EPS || minDelta > EPS : minDelta > EPS);
      return {
        valid,
  ...
  var FloatValidator_default = FloatValidator;

  // js/validation/DoubleRange.js
  var INCLUSIVE_NAME = "INCLUSIVE";
  var DoubleRange = class {
  ...
  var DoubleRange_default = DoubleRange;

  // js/validation/ClientRangeValidator.js
  var ClientRangeValidator = class {
    validate(element, value) {
      let range2 = new DoubleRange_default(element.data("range"));
      let validator = new FloatValidator_default(range2);
      let { valid, message } = validator.check(value);
      if (!valid)
        throw { summary: message, detail: message };
    }
  };
  var ClientRangeValidator_default = () => {
    const doubleValidatorText = 'Value "{0}" is not a number';
    PrimeFaces.locales["en"].messages["jakarta.faces.converter.DoubleConverter.DOUBLE"] = PrimeFaces.locales["en"].messages["jakarta.faces.converter.DoubleConverter.DOUBLE_detail"] = doubleValidatorText;
    PrimeFaces.validator["RangeExternalValidated"] = new ClientRangeValidator();
  };

  // js/views/MainPage.js
  var MainPage = class {
    #window;
    #store;
    #plot;
    ...
    #disableTimeout;
    constructor() {
      ClientRangeValidator_default();
      this.#window = $(window);
      this.#store = new PointResultStorage_default();
      this.#plot = new FigureDisplay_default(
